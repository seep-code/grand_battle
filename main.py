    ...
    def changeCondition(self, position):
        ...
        if self.level_button:
            if position[0] in range(self.rect.left, self.rect.right) and position[1] in range(self.rect.top,
                ...
                time = data[38 + c_d * 12 + l_b * 4][0:2] + " hr " + data[39 + c_d * 12 + l_b * 4][0:2] + " min " + \
                       data[40 + c_d * 12 + l_b * 4][0:2] + " sec"
                if data[c_d * 4 + l_b][0] == '0':
                    status = "not completed"
                if data[38 + c_d * 12 + l_b * 4][0:2] == "00" and data[39 + c_d * 12 + l_b * 4][0:2] == "00" and \
                        data[40 + c_d * 12 + l_b * 4][0:2] == "00":
                    time = "-"
...
class Character(pygame.sprite.Sprite):
    ...
    def move(self, move_left, move_right):
        ...
        if self.lives == 4:
            SCREEN.blit(l_4, (1435, 30))
        if self.lives == 3:
            SCREEN.blit(l_3, (1435, 30))
        if self.lives == 2:
            SCREEN.blit(l_2, (1435, 30))
        if self.lives == 1:
            SCREEN.blit(l_1, (1435, 30))
...
class Turret(pygame.sprite.Sprite):
    ...
    def shoot(self):
        if self.look_direction == 1:
            return EnemyBullet(self.rect.x + self.size[0], self.rect.y + self.size[1] // 3,
                               self.bullet_speed, self.look_direction)
        return EnemyBullet(self.rect.x, self.rect.y + self.size[1] // 3, self.bullet_speed,
                           self.look_direction)
...
def level_01():
    ...
    screen_scroll = 0
    player = Character(600, 770, CHARACTER_SPEED, CHARACTER_HEALTH)
    ...
    if completed:
        if TURRETS_DESTROYED in range(12, 14):
            if data[c_d * 4][0] == '3':
                if int(data[38 + c_d * 12 + 0][:2]) * 3600 + int(data[39 + c_d * 12 + 0][:2]) * 60 + int(data[40 + c_d * 12 + 0][:2]) > ticks / 1000:
    ...
    while True:
        ...
        retry_button = Button(image=pygame.image.load("Resources/Textures/restart_button_disabled.png"),
                                       image_path="Resources/Textures/restart_button_disabled.png", pos=(710, 700))
        ...
        return_to_level_menu = Button(image=pygame.image.load("Resources/Textures/exit_button_disabled.png"),
                                       image_path="Resources/Textures/exit_button_disabled.png", pos=(1110, 700))
...
def level_02():
    ...
    if completed:
        if TURRETS_DESTROYED in range(12, 14):
            if data[c_d * 4 + 1][0] == '3':
                if int(data[38 + c_d * 12 + 4][:2]) * 3600 + int(data[39 + c_d * 12 + 4][:2]) * 60 + int(data[40 + c_d * 12 + 4][:2]) > ticks / 1000:
                    data[38 + c_d * 12 + 4] = str(hours) + '\n'
                    data[39 + c_d * 12 + 4] = str(minutes) + '\n'
                    data[40 + c_d * 12 + 4] = str(seconds) + '\n'
            data[c_d * 4 + 1] = '3\n'
        elif TURRETS_DESTROYED in range(8, 12) and data[c_d * 4 + 1][0] != '3':
            if data[c_d * 4 + 1][0] == '2':
                if int(data[38 + c_d * 12 + 4][:2]) * 3600 + int(data[39 + c_d * 12 + 4][:2]) * 60 + int(data[40 + c_d * 12 + 4][:2]) > ticks / 1000:
                    data[38 + c_d * 12 + 4] = str(hours) + '\n'
                    data[39 + c_d * 12 + 4] = str(minutes) + '\n'
                    data[40 + c_d * 12 + 4] = str(seconds) + '\n'
            data[c_d * 4 + 1] = '2\n'
        elif TURRETS_DESTROYED in range(0, 8) and data[c_d * 4 + 1][0] != '2' and data[c_d * 4 + 1][0] != '3':
            if data[c_d * 4 + 1][0] == '1':
                if int(data[38 + c_d * 12 + 4][:2]) * 3600 + int(data[39 + c_d * 12 + 4][:2]) * 60 + int(data[40 + c_d * 12 + 4][:2]) > ticks / 1000:
                    data[38 + c_d * 12 + 4] = str(hours) + '\n'
                    data[39 + c_d * 12 + 4] = str(minutes) + '\n'
...
def level_03():
    ...
    if completed:
        if TURRETS_DESTROYED in range(12, 14):
            if data[c_d * 4 + 2][0] == '3':
                if int(data[38 + c_d * 12 + 8][:2]) * 3600 + int(data[39 + c_d * 12 + 8][:2]) * 60 + int(data[40 + c_d * 12 + 8][:2]) > ticks / 1000:
...
def endless_mode():
    ...
    for i in range(18):
        for j in range(turrets_update, len(MAP_MATRIX[0])):
            if MAP_MATRIX[i][j] == 'T':
                turret_group.add(Turret(j * 50 + 290, i * 50 + 50, 3, 4))

    map_last_update = pygame.time.get_ticks()
    ...
    while playing:
        ...
        if int((ticks - map_last_update) / 1000 % 60) >= 20:
            map_last_update = ticks
