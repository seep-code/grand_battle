...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self, change, enemy_group):
...
class Turret(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y, look_direction, health):
        pygame.sprite.Sprite.__init__(self)
...
def level_01():
    ...
    for i in range(18):
        for j in range(120):
            if MAP_MATRIX[i][j] == 'T':
                turret_group.add(Turret(j * 50 + 290, i * 50 + 50, -1, TURRET_HEALTH))
    ...
    while playing:
        ...
        portal.draw()
        player.draw()
        bullet_group.update(change, turret_group)
