...
class Character(pygame.sprite.Sprite):
    def __init__(self, x, y, velocity, lives):
        ...
        self.image = pygame.image.load('Resources/Textures/character_test.png')
            ...
            for y in range(len(MAP_MATRIX[x])):
                ...
                if block == 'L':
                    if tile.colliderect(self.rect.x, self.rect.y, self.width,
                        ...
                        if 240 + y * 50 < self.rect.centerx < 240 + y * 50 + 50:
                            self.ladder = True
                            self.air = True
                            self.velocity_y = 0
                            y_change += -2
    ...
    def shoot(self):
        if self.look_direction == 1:
            return Bullet(self.rect.x + self.size[0], self.rect.y + self.size[1] // 3, 6, self.look_direction)
        return Bullet(self.rect.x, self.rect.y + self.size[1] // 3, 6, self.look_direction)
...
def level_background(lvl_n):
    ...
    for i in range(18):
        for j in range(120):
            ...
            elif MAP_MATRIX[i][j] == 'T':
                CANVAS.blit(turret_image, (j * 50 + screen_scroll, i * 50))
